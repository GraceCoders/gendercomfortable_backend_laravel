...
@section('main-container')
    ...
    <section class="third_sec bg-light">
        <div class="container " id="largeimg1">
            @if ($data->thumbnail)
                <img class="img-fluid" id="largeimg" class="image"
                    src="{{ asset('storage/' . $data->thumbnail) }}" alt="...">
            @else
                <img class="img-fluid" id="largeimg" class="image"
                    src="{{ asset('assets/slicing_web/logo.svg') }}" alt="...">
            @endif
    ...
    <nav class="third-sec bg-light">
        <div class="container" id="nav_btn">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link bg-light" id="profile-tab" type="button" role="tab" data-bs-toggle="modal"
            ...
            <div class="container">
                ...
                @if ($data['lessons'])
                    @foreach ($data['lessons'] as $value)
                        ...
                        <p class="donec">
                            @php
                                $open = ProtoneMedia\LaravelFFMpeg\Support\FFMpeg::fromDisk('public')->open($value->media);
                                dd($open);
                            @endphp
                            <a href="{{ route('media.player', $value->id) }}">
                                @if ($value->media_type == 'application/pdf')
                                    <img class="img-fluid" class="image"
                                        src="{{ asset('assets/slicing_web/pdf.svg') }}" height="26px" width="26px"
                                        alt="...">
                                @elseif($value->media_type == 'image/jpeg' || $value->media_type == 'image/png')
                                    <img class="img-fluid" class="image"
                                        src="{{ asset('assets/slicing_web/home.svg') }}" height="26px" width="26px"
                                        alt="...">
                                @else
                                    <img class="img-fluid" class="image"
                                        src="{{ asset('assets/slicing_web/video.svg') }}" alt="...">
                                @endif
                            </a>
                            {{ $value->lession_name }}
                        </p>
                                ...
                                @foreach ($purchaseCourse as $value)
                                    <div class=" col-lg-4 col-12">
                                        <div class="comapany1">
                                            ...
                                            <p>Unique course access key:<br> <span
                                                    id="digit_val">{{ $value->purchase_key }}
                                                </span></p>
                                        </div>
                                    </div>
                                @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <a href="{{ route('admin.course.delete', $data->id) }}"><button type="button"
